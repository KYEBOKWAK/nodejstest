...
module.exports = {
  save: function(key_main, value1, expire, callback){
    /*
    const client = redis.createClient('6379', "ec2-3-112-50-176.ap-northeast-1.compute.amazonaws.com");
    //https://redis.crowdticket.kr
    // const client = redis.createClient("54.249.28.251:6379");
    ...
    if(expire === 0)
    {
      client.set(key_main, value1, (error) => {
        console.log("ERROR!!?");
        console.log(error);
      });
    }
    ...
    {
      client.set(key_main, value1, 'EX', expire, (error) => {
        console.log("!?!!? ERROR  !!?");
        console.log(error);
      });
    }
    ...
    */
    ...
    new Promise(function(resolve, reject){
      // const client = redis.createClient(process.env.REDIS_PORT, process.env.REDIS_URL, {password: process.env.REDIS_PW});
      const client = redis.createClient(process.env.REDIS_CHATTING_PORT, process.env.REDIS_CHATTING_URL);
      if(expire === 0)
      {
