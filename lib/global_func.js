...
module.exports = {
  ...
  sendKakaoAlimTalk(datas){
    ...
    if(_templateCode === 'CTSTORE01a'){
    ...
    else if(_templateCode === 'Kalarm17v1'){
      _content = `[1:1 문의 알림]\n${data.creator_name} 님, 1:1 문의 알림이 등록되었어요!\n\n■ 상품명 : ${data.item_name}\n\n- 문의 내용 -\n■ 문의제목 : ${data.ask_title}\n■ 작성자 : ${data.ask_name}\n■ 작성일 : ${data.ask_date}`

      buttons = [
          {
              type: "WL",
              name: "확인하기",
              linkMobile: `https://${data.link_url}`,
              linkPc: `https://${data.link_url}`,
          }
      ];

      _smsContent = `[크티] ${data.creator_name} 님, 1:1 문의 알림이 등록되었어요!`;
    }
    else if(_templateCode === 'Kalarm18v1'){
      _content = `[1:1 문의 답변 알림]\n${data.ask_name} 님, 1:1 문의 답변이 등록되었어요!\n\n■ 플레이스명 : ${data.place_name}\n■ 상품명 : ${data.item_name}\n\n- 문의 내용 -\n■ 문의제목 : ${data.ask_title}\n■ 작성일 : ${data.ask_date}`
      ...
      _smsContent = `[크티] ${data.ask_name} 님, 1:1 문의 답변이 등록되었어요!`;
    }
    ...
    headers: 
    {
        ...
        'x-ncp-apigw-signature-v2': this.makeSignature(time, `/alimtalk/v2/services/${process.env.NAVER_BIZ_MESSAGE_SERVICE_ID}/messages`)
    },
    method: 'POST',
  ...
  sendSMS(to, countryCode, content, callback) {
    ...
    headers: 
    {
        ...
        'x-ncp-iam-access-key': process.env.NAVER_API_ACCESS_KEY,
        'x-ncp-apigw-timestamp': time,
        'x-ncp-apigw-signature-v2': this.makeSignature(time, `/sms/v2/services/${process.env.NAVER_SMS_SERVICE_ID}/messages`)
    },
    method: 'post',
