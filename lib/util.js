...
const language = require('./language/email/language-email.json');
...
module.exports = {
  ...
  getStoreOrderWaitTimeSec: (date) => {
    const waitEndTime = moment_timezone(date).add(global.pay.WAIT_STORE_ORDER_TIME_MIN, 'm');
    let gapMiliSecTime = moment_timezone(waitEndTime).diff(moment_timezone(), 'milliseconds');
    gapMiliSecTime = gapMiliSecTime / 1000;
  ...
  isExpireTime: (time) => {
    ...
    let expireGap = moment_timezone(time).diff(moment_timezone());
    if(expireGap < 0){
      return true;
    }
  ...
  isSaling: (sale_start_at) => {
    ...
    let isSaling = moment_timezone(sale_start_at).diff(moment_timezone());
    if(isSaling < 0){
      return true;
    }
  ...
  getWaitTimeMinWithText: (date) => {
    const waitEndTime = moment_timezone(date).add(global.pay.WAIT_TIME_MIN, 'm');
    let gapMiliSecTime = moment_timezone(waitEndTime).diff(moment_timezone());
    gapMiliSecTime = gapMiliSecTime / 1000;
  ...
  getShowTimePassByNow: (target_date) => {
    let gapMiliSecTime = moment_timezone(target_date).diff(moment_timezone());
    gapMiliSecTime = gapMiliSecTime / 1000;
  ...
  getPayStoreNewMerchant_uid: function(store_id, user_id){
    ...
    return 's_'+store_id+'_u_'+user_id+'_'+this.getTimeSec()+_appType;
  },
  ...
  isPrjectFinished: function(funding_closing_at){
    ...
    let gapMiliSecTime = moment_timezone(funding_closing_at).diff(moment_timezone());
    gapMiliSecTime = gapMiliSecTime / 1000;
  ...
  isTicketShowFinished: function(show_date){
    ...
    let gapMiliSecTime = moment_timezone(show_date).diff(moment_timezone());
    gapMiliSecTime = gapMiliSecTime / 1000;
  ...
  getStr(strKey, language_code, replaces = []) {
    ...
    else if(language.language[stringKey] === undefined){
     text = stringKey;
    }else if(language.language[stringKey][language_code] === undefined || language.language[stringKey][language_code] === null){
     text = stringKey;
    }else{
     text = language.language[stringKey][language_code];
    }
  
    if(Array.isArray(replaces) && replaces.length > 0){
     for(let i = 0 ; i < replaces.length ; i++){
       const data = replaces[i];
       text = text.replace('%s', data);
     }
    }
