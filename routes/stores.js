...
router.post('/any/item/list', function(req, res){
  ...
  let querySelect = mysql.format("SELECT item.id, item.store_id, store.alias, price, item.title, item.img_url, nick_name FROM items AS item LEFT JOIN stores AS store ON item.store_id=store.id LEFT JOIN users AS user ON store.user_id=user.id WHERE item.state=? ORDER BY item.id DESC LIMIT ? OFFSET ?", [Types.item_state.SALE, limit, skip]);
...
router.post("/manager/order/list", function(req, res){
  ...
  if(sort_state === 0 && sort_item_id === -1){
    ...
    querySelect = mysql.format("SELECT orders_item.state, orders_item.count, orders_item.created_at, orders_item.id, orders_item.store_id, orders_item.total_price, item.title FROM orders_items AS orders_item LEFT JOIN items AS item ON orders_item.item_id=item.id WHERE orders_item.store_id=? AND orders_item.item_id=? ORDER BY orders_item.id DESC LIMIT ? OFFSET ?", [store_id, sort_item_id, limit, skip]);
  }else {
