...
const ALARM_DOWNLOAD_ORDER_CHECK_MIN = 24;
...
function alarmDownloadOrderCheck() {
  const nowDate = moment_timezone(`2021-07-16 ${ALARM_DOWNLOAD_ORDER_CHECK_HOUR}:${ALARM_DOWNLOAD_ORDER_CHECK_MIN}:00`).format(`YYYY-MM-DD ${ALARM_DOWNLOAD_ORDER_CHECK_HOUR}:${ALARM_DOWNLOAD_ORDER_CHECK_MIN}:00`);
...
function storeTierEnterCheck(){
  ...
  const querySelect = mysql.format("SELECT store.id AS store_id, store.id, store.title, store.state, store.tier, store.contact, store.email, store.account_name, store.account_number, store.account_bank, store.view_count FROM (SELECT store.id, store.title, store.state, store.tier, store.contact, store.email, store.account_name, store.account_number, store.account_bank, store.view_count FROM stores AS store LEFT JOIN items AS item ON item.store_id=store.id GROUP BY store.id HAVING COUNT(CASE WHEN item.state=? THEN 1 END)=0 ) AS store LEFT JOIN orders_items AS orders_item ON store.id=orders_item.store_id WHERE store.state=? AND (store.tier<>? OR store.tier IS NULL) GROUP BY store.id HAVING COUNT(CASE WHEN orders_item.state<99 THEN 1 END)=0", [Types.item_state.SALE, Types.store.STATE_APPROVED, Types.tier_store.enter]); //enter
...
function storeTierOpenCheck(){
  ...
  const querySelect = mysql.format("SELECT store.id AS store_id, store.id, store.title, store.state, store.tier, store.contact, store.email, store.account_name, store.account_number, store.account_bank, store.view_count FROM (SELECT store.id, store.title, store.state, store.tier, store.contact, store.email, store.account_name, store.account_number, store.account_bank, store.view_count FROM stores AS store LEFT JOIN items AS item ON item.store_id=store.id GROUP BY store.id HAVING COUNT(CASE WHEN item.state=? THEN 1 END)>0 ) AS store LEFT JOIN orders_items AS orders_item ON store.id=orders_item.store_id WHERE store.state=? AND (store.tier<>? OR store.tier IS NULL) GROUP BY store.id HAVING COUNT(CASE WHEN orders_item.state<99 THEN 1 END)=0", [Types.item_state.SALE, Types.store.STATE_APPROVED, Types.tier_store.open]); //open
...
function storeTierSaleCheck(){
  ...
  const querySelect = mysql.format("SELECT store.id AS store_id, store.id, store.title, store.state, store.tier, store.contact, store.email, store.account_name, store.account_number, store.account_bank, store.view_count FROM (SELECT store.id, store.title, store.state, store.tier, store.contact, store.email, store.account_name, store.account_number, store.account_bank, store.view_count FROM stores AS store LEFT JOIN items AS item ON item.store_id=store.id GROUP BY store.id HAVING COUNT(CASE WHEN item.state=? THEN 1 END) > 0 ) AS store LEFT JOIN orders_items AS orders_item ON store.id=orders_item.store_id LEFT JOIN items AS item ON orders_item.item_id=item.id WHERE store.state=? AND (store.tier<>? OR store.tier IS NULL) GROUP BY store.id HAVING COUNT(CASE WHEN orders_item.state<99 THEN 1 END)>0 AND COUNT(CASE WHEN orders_item.state<99 THEN 1 END)<=2", [Types.item_state.SALE, Types.store.STATE_APPROVED, Types.tier_store.sale]); //open
...
function storeTierSaleKeepCheck(){
  ...
  const querySelect = mysql.format("SELECT store.id AS store_id, store.id, store.title, store.state, store.tier, store.contact, store.email, store.account_name, store.account_number, store.account_bank, store.view_count FROM (SELECT store.id, store.title, store.state, store.tier, store.contact, store.email, store.account_name, store.account_number, store.account_bank, store.view_count FROM stores AS store LEFT JOIN items AS item ON item.store_id=store.id GROUP BY store.id HAVING COUNT(CASE WHEN item.state=? THEN 1 END) > 0 ) AS store LEFT JOIN orders_items AS orders_item ON store.id=orders_item.store_id LEFT JOIN items AS item ON orders_item.item_id=item.id WHERE store.state=? AND (store.tier<>? OR store.tier IS NULL) GROUP BY store.id HAVING COUNT(CASE WHEN orders_item.state<99 THEN 1 END)>=3", [Types.item_state.SALE, Types.store.STATE_APPROVED, Types.tier_store.sale_keep]); //open
...
function storeTierBreakCheck(){
  ...
  const querySelect = mysql.format("SELECT store.id AS store_id, store.id, store.title, store.state, store.tier, store.contact, store.email, store.account_name, store.account_number, store.account_bank, store.view_count FROM (SELECT store.id, store.title, store.state, store.tier, store.contact, store.email, store.account_name, store.account_number, store.account_bank, store.view_count FROM stores AS store LEFT JOIN items AS item ON item.store_id=store.id GROUP BY store.id HAVING COUNT(CASE WHEN item.state=? THEN 1 END)=0 ) AS store LEFT JOIN orders_items AS orders_item ON store.id=orders_item.store_id WHERE store.state=? AND (store.tier<>? OR store.tier IS NULL) GROUP BY store.id HAVING COUNT(CASE WHEN orders_item.state<99 THEN 1 END)>0", [Types.item_state.SALE, Types.store.STATE_APPROVED, Types.tier_store.break]); //break
